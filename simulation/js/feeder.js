...
function changeSwitch(switchId, status) {
  ...
  if (status == true) {
    switchh.classList.remove("switch--open");
  } else {
    switchh.classList.add("switch--open");
  }
  // document.getElementById(switchId).src = status == true ?  : "./images/switchOpen.png";
  if (
    switchId == "f11" ||
    switchId == "f12" ||
    switchId == "f13" ||
...
function updateTable(index, status) {
  data[index].counter =
    status == true ? data[index].counter + 1 : data[index].counter - 1;
  if (index == 0) {
    document.getElementById("mi1").innerHTML =
      data[index].counter == -1
        ? data[index].m1[3]
        : data[index].m1[data[index].counter];
    document.getElementById("mp1").innerHTML =
      data[index].counter == -1
        ? data[index].m1[3]
        : data[index].p1[data[index].counter];
    document.getElementById("meter1Load").innerHTML =
      data[index].counter == 2
        ? "FULL LOAD"
        : data[index].counter == 3
        ? "TRIP"
        : "";
    // document.getElementById("leftBlue").style.opacity =
    //   data[index].counter == 3 ? 1 : 0;
    if (data[index].counter == 3) {
      showPopup("CB6 is Tripped");
      document.getElementById("leftBlue").classList.remove("opacityy");
    } else {
      document.getElementById("leftBlue").classList.add("opacityy");
    }
    updateMeterTable3();
  } else {
    document.getElementById("mi2").innerHTML =
      data[index].counter == -1
        ? data[index].m2[3]
        : data[index].m2[data[index].counter];
    document.getElementById("mp2").innerHTML =
      data[index].counter == -1
        ? data[index].m2[3]
        : data[index].p2[data[index].counter];
    document.getElementById("meter2Load").innerHTML =
      data[index].counter == 2
        ? "FULL LOAD"
        : data[index].counter == 3
        ? "TRIP"
        : "";
    document.getElementById("rightBlue").style.opacity =
      data[index].counter == 3 ? 1 : 0;
    if (data[index].counter == 3) {
      showPopup("CB7 is Tripped");
      document.getElementById("rightBlue").classList.remove("opacityy");
    } else {
      document.getElementById("rightBlue").classList.add("opacityy");
    }
    updateMeterTable3();
  }
...
function showPopup(text) {
  popupVisible = true;
  document.getElementById("parentPopup").classList.remove("displayy");
...
function closeModal(ev) {
  ev.preventDefault();
  popupVisible = false;
  document.getElementById("parentPopup").classList.add("displayy");
}
...
function resetSwitch() {
  // if (data[0].counter == 3 || data[1].counter == 3) return;
  ...
  data.forEach((d) => {
    ...
    d.switch.forEach((e) => {
      e.status = false;
      document.getElementById(e.id).classList.add("switch--open");
    });
  });
